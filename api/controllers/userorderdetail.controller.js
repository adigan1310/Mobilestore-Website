...
module.exports.orderAddOneByUserId = function(req, res)  {
  ...
  console.log("jsoncart",JSON.stringify(newOrderDetail));
  ...
  User
    ...
    .exec(function(err, user) {
      if(err) {
        ...
        var filterCond = {};
        filterCond.$or = [];
        console.log("#items",newOrderDetail.product.length);
           ...
           .exec(function(err, mobile){
             ...
             for(var i = 0; i < newOrderDetail.product.length; i++)  {
               for(var j = 0; j < mobile.length; j++) {
                  if(newOrderDetail.product[i].productId == mobile[j]._id) {
                   if(mobile[j].availableQuantity >= newOrderDetail.product[i].quantity) {
